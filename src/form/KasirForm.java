...
public class KasirForm extends javax.swing.JFrame {
    ...
    private void generateIDKasir() {
    try {
        Connection conn = koneksi.Koneksi.getKoneksi(); 
        Statement stmt = conn.createStatement();
        ResultSet rs = stmt.executeQuery("SELECT id_kasir FROM kasirform ORDER BY id_kasir DESC LIMIT 1");

        if (rs.next()) {
            String lastId = rs.getString("id_kasir"); 
            int number = Integer.parseInt(lastId.substring(5)); 
            number++;
            String newId = String.format("KASIR%03d", number); 
            txtID.setText(newId);
        } else {
            txtID.setText("KASIR001"); 
        }
    } catch (Exception e) {
...
try {
    ...
    PreparedStatement pKasir = c.prepareStatement(sql);
        pKasir.setString(1, id);
        pKasir.setString(2, nama);
        pKasir.setString(3, jk);
        pKasir.setString(4, telp);
        pKasir.setString(5, alamat);
        pKasir.executeUpdate();
    
    String sqlLogin = "INSERT INTO login (username, password, id_kasir) VALUES (?, ?, ?)";
        PreparedStatement pLogin = c.prepareStatement(sqlLogin);
        pLogin.setString(1, username);
        pLogin.setString(2, password);
        pLogin.setString(3, id);
